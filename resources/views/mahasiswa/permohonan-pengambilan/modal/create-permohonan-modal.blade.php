        ...
        <div class="modal-dialog" role="document">
            ...
            <form action="{{ route('create-permohonan-mahasiswa') }}" method="post">
                <div class="modal-body">
                    ...
                    <select name="keperluan" id="keperluan" class="form-control mb-3">
                        <option value="skripsi">Skripsi</option>
                        <option value="mata_kuliah">Mata Kuliah</option>
                    </select>
                    <div id="judul_skripsi" class="d-none">
                        <label for="judul_skripsi">Judul Skripsi</label>
                        <textarea name="judul_skripsi" id="judul_skripsi" cols="30" rows="2" class="form-control mb-3"></textarea>
                    </div>
                    <div id="matkul" class="d-none">
                        <label for="dosen">Dosen Pengampu</label>
                        <select name="dosen" id="dosen" class="form-control mb-3">
                           @foreach ($dosen as $detail)
                                <option value="{{ $detail->dataDosen->nidn }}">{{ $detail->dataDosen->nama }}</option>
                            @endforeach
                        </select>
                    </div>
