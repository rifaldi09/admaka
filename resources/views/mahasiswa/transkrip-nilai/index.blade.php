...
<div class="card mt-3">
    <div class="card-header">
        ...
        <button type="button" class="btn btn-primary float-right btn-sm" data-toggle="modal" data-target="#transkripModal">
                    ...
                    <td>
                        <nobr>
                            @if ($detail->status == 'Penerbitan')
                                @if($detail->filePermohonan && $detail->filePermohonan->isNotEmpty())
                                    @foreach ($detail->filePermohonan as $file)
                                        <a href="{{ asset('storage/transkrip/' . basename($file->path)) }}" target="_blank" class="btn btn-default text-primary">
                            ...
                            @elseif ($detail->status == 'Ditolak')
                                <button type="button" class="btn btn-default text-warning btn-edit" data-toggle="modal" data-target="#editTranskripModal-{{ $detail->id_transkrip }}">
