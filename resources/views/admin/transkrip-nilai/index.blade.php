...
@section('title', 'Surat Permohonan Teranskrip Nilai')

@section('content')
<div class="card mt-3">
    <div class="card-header">
        <h1 class="card-title font-weight-bold">Surat Permohonan Teranskrip Nilai</h1>
    </div>
    <div class="card-body">
                ...
                <div class="mt-3">
                    ...
                    <x-adminlte-datatable id="table1" :heads="$heads">
                        ...
                        <tr>
                            ...
                            <td>
                                <nobr>
                                    <button class="btn btn-primary btn-detail-terima btn-sm " data-toggle="modal"
                                        data-target="#detailtranskripModal-{{ $tp->id_transkrip }}" title="Detail">
                                    ...
                                    <button class="btn btn-danger btn-tolak btn-sm" data-toggle="modal"
                                        data-target="#tolakTranskrip-{{  $tp->id_transkrip }}" title="Tolak">
                                        <i class="fa-solid fa-circle-info"></i> Tolak
                                    </button>
                                    @include('admin.transkrip-nilai.modal.tolaktp', [
                                    'id' => $tp->id_transkrip,'route' =>
                                    'tolak-transkrip'
                                    ])
                                </nobr>
                            </td>
                            ...
                            <td>
                                <nobr>
                                    ...
                                    <button class="btn btn-success btn-sm btn-upload" data-toggle="modal"
                                        data-target="#uploadTranskripModal-{{ $tp->id_transkrip }}" title="Upload">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $(document).on('click', '.btn-setujui, .tolak', function(e) {
        ...
        const isAtprove = $(this).hasClass('btn-setujui');

        // Validation for reject form
        if (!isAtprove && !form.checkValidity()) {
            form.reportValidity();
            return;
        }
        ...
        Swal.fire({
            ...
            confirmButtonColor: isAtprove ? '#14A44D' : '#d33',
            cancelButtonColor: '#3085d6',
            confirmButtonText: isAtprove ? 'Ya, setujui suratnya' : 'Ya, tolak suratnya',
    ...
    $(document).on('click', '.penerbitan', function(e) {
        e.preventDefault();
        const form = $(this).closest('form')[0];
        const isAtprove = $(this).hasClass('.penerbitan');
